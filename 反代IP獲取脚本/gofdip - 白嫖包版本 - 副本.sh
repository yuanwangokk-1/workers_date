...
# 本脚本为全自动下载白嫖的反代包，自动筛选有效反代并更新到域名，人人都能拥有自己的反代域名啦。
...
hostnames=
#################################################反代设置#######################################################
# --先择域（31898甲骨文和45102阿里云）都要就留空--
asn=""
#
# --端口，都要就留空，建议留空，反代IP不需要指定端口，由于是网页验证反代，可以考虑使用80端口，减少测试的IP量--
port="80"
#
...
echo "===============================开始下载白嫖的反代IP包=================================="
# 定义下载链接和保存路径
download_url="${DL}https://zip.baipiao.eu.org/"
...
###################################################################################################
# 循环尝试下载
while [ $current_attempt -lt $max_attempts ]
do
    # 下载文件
    ...
    current_attempt=$((current_attempt+1))
        if [ $current_attempt -eq $max_attempts ]; then
        	echo "IP包无法下载，程序已退出，请检查网络和代理状态"
        	exit 1;
        	else
        	echo "IP包下载失败，重试下载，请检查网络和代理状态"
        fi
    fi
done
# 删除原来的txt文件夹内容
...
# 遍历指定目录中的所有文本文件
find "$DIRECTORY" -type f -name "*.txt" -print0 | while IFS= read -r -d '' FILEPATH; do
    # 创建临时文件
    tempFILEPATH=$(mktemp)
    
    # 使用awk去除重复行，并将结果写入临时文件
    awk '!seen[$0]++' "$FILEPATH" > "$tempFILEPATH"
